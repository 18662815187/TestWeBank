	...
	<form enctype="multipart/form-data" action="work/save" method="post"
		...
		<table class="table table-bordered table-hover definewidth m10">
			...
			<tr>
				...
				<td><input type="text" name="customer.wechatwhite"
					value="${customer.wechatwhite}" /></td>
			</tr>
			<tr>
				<td width="10%" class="tableleft">QQ白名单openId</td>
				<td><input type="text" name="customer.qqwhile"
					value="${customer.qqwhile}" /></td>
				...
				<td><select name="customer.openchannel">
						<c:forEach items="${datalist }" var="d">
							<c:if test="${d.pid==2 }">
								<option value="${d.id }"
									${d.id==customer.openchannel?'selected':'' }>${d.name }</option>
							</c:if>
						</c:forEach>
				</select></td>

				<td width="10%" class="tableleft">客群分类</td>
				<td><input type="text" name="customer.customertype"
			...
			<tr>
				...
				<td><select name="customer.comeinchannel">
						<c:forEach items="${datalist}" var="d">
							<c:if test="${d.pid==3}">
								<option value="${d.id }"
									${d.id==customer.comeinchannel?'selected':'' }>${d.name }</option>
							</c:if>
						</c:forEach>
				</select></td>

				<td width="10%" class="tableleft">来电号码</td>
				<td><input type="text" name="customer.callphone"
					value="${customer.callphone}" /></td>
			...
			<tr>
				...
				<td colspan="5"><input type="text" name="username"
					value="${data.memo}" /></td>
			...
			<div id="myTabContent" class="tab-content definewidth m20">
				<c:choose>
					<c:when test="${work.status!=2 }">
						...
						<c:if test="${sessionScope.user.userid!=work.handleperson }">
